...
import { updateQuantity, removeFromCart, clearCart } from '../redux/cartSlice';  // Assuming clearCart action exists
...
const CartPage = () => {
  ...
  // Handle stock updates on checkout
  const handleCheckout = () => {
    cartItems.forEach(item => {
      const newStock = item.inStock - item.quantity;
      dispatch(updateQuantity({ id: item.id, quantity: newStock }));
    });
    ...
    <div className="container mx-auto p-6">
      {isCheckoutComplete ? (
        ...
        <div className="bg-white shadow-lg rounded-lg p-6">
          {cartItems.map(item => (
            <div key={item.id} className="flex flex-col md:flex-row items-center justify-between p-4 mb-4 border-b rounded-lg">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className={item.inStock > 0 ? "text-green-500" : "text-red-500"}>
                    {item.inStock > 0 ? "In Stock" : "Out of Stock"}
                  </p>
                  {item.inStock < 3 && item.inStock > 0 && (
                    <p className="text-yellow-500">Hurry up! Only {item.inStock} left in stock.</p>
                  )}
                </div>
              </div>
